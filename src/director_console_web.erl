...
loop(Req) ->
	...
	{static, File} ->
	    "/" ++ StaticFile = File,
	    Req:serve_file(StaticFile,director_console_deps:local_path(["www"]));
	{error,_} ->
	    Req:respond({500,[],"Server Error"})
    end.

render_template(ViewFile,Data) -> 
    FullPathToFile = filename:join([director_console_deps:local_path(["views"]),ViewFile]),
